    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if (Test-Path $apkPath) {
                ...
                Write-Host "已复制APK到根目录: $((Get-Item $rootApkPath).FullName)" -ForegroundColor Green
            } else {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if (Test-Path $aabPath) {
                # 复制AAB到根目录并重命名
                $rootAabPath = "./Lumina-v1.0.aab"
                Copy-Item -Path $aabPath -Destination $rootAabPath -Force
                
                Write-Host "AAB应用包构建成功!" -ForegroundColor Green
                Write-Host "AAB位置: $((Get-Item $aabPath).FullName)" -ForegroundColor Green
                Write-Host "已复制AAB到根目录: $((Get-Item $rootAabPath).FullName)" -ForegroundColor Green
            } else {
